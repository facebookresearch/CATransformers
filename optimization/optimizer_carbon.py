...
from configurations import TEXT_MODEL_PARAMS, VISION_MODEL_PARAMS, HW_PARAMS, NUM_TRIALS, AREA_CONSTRAINT, LATENCY_CONSTRAINT, AREA_CONSTRAINT_VALUE, LATENCY_CONSTRAINT_VALUE
import csv, os
import pandas as pd
...
def pareto_frontier(df, area_constraint, latency_constraint, save_name):
    home_dir = os.getcwd()
    ...
    plt.scatter(df['carbon'], df['accuracy'], c='blue', alpha=0.5, label='All points')
    plt.scatter(frontier_points['carbon'], frontier_points['accuracy'], c='red', label='Pareto frontier')
    plt.xlabel('Carbon')
    plt.ylabel('Accuracy')
...
def optimize(run_name):
    ...
    ax_client.create_experiment(
        ...
        objectives={
            ...
            "accuracy": ObjectiveProperties(minimize=False, threshold=0.2),
            "carbon": ObjectiveProperties(minimize=True,threshold=6.0),
        },
        outcome_constraints=[
            AREA_CONSTRAINT,
            LATENCY_CONSTRAINT
    ...
    # Compute Pareto frontier
    frontier_points = pareto_frontier(df, area_constraint, latency_constraint, run_name)